...
const mcp = new Server(
  { name: 'telegram', version: '1.0.0' },
  {
    ...
    instructions: [
      ...
      '',
      'reply accepts file paths (files: ["/abs/path.png"]) for attachments. Choose between reply (text) and voice_reply (voice+caption) based on content: use voice_reply for short conversational responses (confirmations, brief answers, casual chat); use text reply for anything with code, file paths, tables, lists, technical details, long explanations, or structured content. If the user explicitly requests voice or text, follow their preference. Use react to add emoji reactions, and edit_message for interim progress updates. Edits don\'t trigger push notifications — when a long task completes, send a new reply so the user\'s device pings.',
      '',
